...
Public Class ListadoSolcitudesvb

    Dim fechaActual As String
    Public Sub CreaResumen(ByVal sql As String, ByVal filtro As String)
        ...
        Try
            ...
            If Not My.Computer.FileSystem.DirectoryExists(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\ReporteLeon\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim) Then

                My.Computer.FileSystem.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\ReporteLeon\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim)
            End If

            Dim cadenafolder As String = (Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\ReporteLeon\" & Year(Now) & acompletacero(Month(Now).ToString(), 2)).Trim

            'Dar propiedades al Documento
            Dim pdfDoc As New Document(iTextSharp.text.PageSize.LETTER, 15.0F, 15.0F, 5.0F, 3.0F)

            'Obtener la ruta donde se va a crear el pdfli
            Dim pdfWrite As PdfWriter = PdfWriter.GetInstance(pdfDoc, New System.IO.FileStream(cadenafolder & "\ListadoSolictud" & filtro & ".pdf", FileMode.Create))
...
#End Region
            ...
            Try
                ...
                ColPiePag1 = New PdfPCell(New Phrase("REPORTE DE SOLICITUDES", Font10N))
                ColPiePag1.Border = 0
                ColPiePag1.HorizontalAlignment = PdfPCell.ALIGN_CENTER
                'ColPiePag1.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                ...
                Dim ColEncFec = New PdfPCell(New Phrase("REPORTE DE SOLICITUDES DEL " & filtro, Font8))
                ColEncFec.Border = 0
                ...
                Dim widthsRec2 As Single() = New Single() {70.0F, 30.0F, 250.0, 110.0F, 110.0F}
                ...
                ColRecibo = New PdfPCell(New Phrase("NO. ", Font7White))
                ColRecibo.Border = 0
                ColRecibo.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                ColRecibo.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                TableRecibos.AddCell(ColRecibo)
                ...
                ColRecibo = New PdfPCell(New Phrase("COLONIA", Font7White))
                ColRecibo.Border = 0
                ColRecibo.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                ColRecibo.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                TableRecibos.AddCell(ColRecibo)

                ColRecibo = New PdfPCell(New Phrase("ESTADO", Font7White))
                ColRecibo.Border = 0
                ColRecibo.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                ColRecibo.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                TableRecibos.AddCell(ColRecibo)
                ...
                ColRecibo = New PdfPCell(New Phrase("TOTAL DE SOLICITUDES", Font10N))
                ColRecibo.Border = 0
                ColRecibo.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                '   ColRecibo.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                TableRecibos.AddCell(ColRecibo)
                ...
                ColRecibo = New PdfPCell(New Phrase(sumaSubtotal, Font10N))
                ColRecibo.Border = 0
                ColRecibo.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                '  ColRecibo.BackgroundColor = New iTextSharp.text.BaseColor(21, 76, 121)
                TableRecibos.AddCell(ColRecibo)
            ...
            Try
                Dim psi As New ProcessStartInfo(cadenafolder & "\ListadoSolictud" & filtro & ".pdf")
